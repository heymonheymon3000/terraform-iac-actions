    ...
    Statement = [
      {
        ...
        Sid    = ""
        Principal = {
          Federated = "${output.aws_iam_openid_connect_provider_arn}"
        }
        Condition = {
          StringEquals = {
            "${output.aws_iam_openid_connect_provider_extract_from_arn}:sub": "system:serviceaccount:default:irsa-demo-sa"
          }
        }
      },
    ]
